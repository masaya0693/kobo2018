...
class uec_foodLoader(data.Dataset):
    def __init__(
        self,
        root,
        split="training",
        ...
        for split in ["training", "Masks"]:
            ...
            if(split == "training"):
                ...
                rootdir=self.root + split, suffix=".jpg"
                )
                self.files[split] = file_list
            else:
                file_list = recursive_glob(
                rootdir=self.root + split, suffix=".png"
                )
                self.files[split] = file_list
    ...
    def __getitem__(self, index):
        img_path = self.files["training"][index].rstrip()
        lbl_path = self.files["Masks"][index].rstrip()
...
if __name__ == "__main__":
    local_path = "/home/y2017/t1710401/semseg/pytorch-semseg-master/uec_food/uecfood100_seg"
    dst = uec_food(local_path, is_transform=True)
